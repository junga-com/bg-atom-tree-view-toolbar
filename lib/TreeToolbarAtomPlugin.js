...
class TreeToolbarAtomPlugin extends BGAtomPlugin {
	constructor(state) {
		...
		// We do idempotent activation in lateActivate so call it in response to the tree-view activating.
		this.watchPackage('tree-view', (pkgName, isActive)=>{if (isActive) this.lateActivate()});

		this.poyfillPagingModes = new TreeViewPageUpDnModesPolyfill();
		atom.config.onDidChange('bg-tree-view-toolbar.enablePgUpDnPatch', {}, ({newValue})=>{this.poyfillPagingModes.enable(newValue)})
	}
...
TreeToolbarAtomPlugin.config =  {
	...
	"properties": {
		...
		"btnRevealCodePatch": {
			...
			"type": "boolean",
			"default": false,
			"title": "Auto Reveal Code Patch",
			"description": "This allows you to use the Auto Reveal button before the [PR #1336](https://github.com/atom/tree-view/pull/1336) has been accepted and is available.  This may or may not work depending on if the atom code has changed since I last updated this package but should do no harm in any case."
		},
		"fontGroup": {
			"order": 50,
			"type": "boolean",
	...
	"enablePgUpDnPatch": {
		...
		"description": "This applies a dynamic patch that adds several types of PgUp/PgDn behaviors for the tree view. This may or may not work depending on if the atom code has changed since I last updated this package but should do no harm in any case. It will be unneccessary if [PR #NNNN](https://github.com/atom/tree-view/pull/NNNN) gets accepted and is available."
	}
}
