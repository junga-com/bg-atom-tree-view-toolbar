...
export class TreeToolbarView extends Component {
	constructor(plugin, isShown, ...p) {
		...
		// if the tree-view is not opened when atom starts but later is opened, we need to install ourselves
	...
	isButtonEnabled(btnName) {
		return (this[btnName]) && (ComponentToEl(this[btnName]).style.display != 'none')
	}
	...
	toggleButton(btnName, state) {
		...
		ComponentToEl(this[btnName]).style.display = (state) ? '' : 'none';
	}
	...
	// package being activated/deactivated, the tree-view item being opened/added or closed/remove from the workspace and when it
	...
	syncWithDOM(uninstall=false) {
		...
		if (shouldBeMounted && !this.isMountedNextToTreeView()) {
			// mount it
			Component.mount(this.treeViewEl.parentNode, this.el, this.treeViewEl);
			...
			// remove it
			this.el.remove()
			//Component.unmount(this.el.parentNode, this.el);
			if (this.treeViewEl)
